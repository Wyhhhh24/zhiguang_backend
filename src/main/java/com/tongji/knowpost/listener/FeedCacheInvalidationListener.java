...
public class FeedCacheInvalidationListener {
     ...
     * - 若可解析到内容的创作者 ID，则同步其 “收到的点赞/收藏” 计数，也就是用户维度的计数；
    ...
    public void onCounterChanged(CounterEvent event) {
        ...
        if ("like".equals(metric) || "fav".equals(metric)) {
            ...
            // 通过内容索引键，获取以该（实体 Id + 时间槽）对应的所有页面缓存键，也就是该知文所存在于哪些页面中
            ...
            // 遍历所有的页面缓存键，进行更新缓存
            // 这里的 key 其实就是 pageKey
            for (String key : keys) {
                // 从本地缓存中，通过 pageKey 分页缓存键获取对应 Key 的分页响应
                FeedPageResponse local = feedPublicCache.getIfPresent(key);
                // 如果本地缓存中的分页响应不为空
                if (local != null) {
                    // 对分页响应进行修改，也就是修改计数，返回修改后的分页响应
     ...
     * - preserveUserFlags=false：写回 Redis 页面 JSON 时不携带用户态标志，也就是 liked/faved 为 null；
     * - 返回新的页面响应快照
     */
