...
public class AuthController {
    ...
    private String extractClientIp(HttpServletRequest request) {
        ...
        // 若都未获取成功，最后走这一步，与当前 Java 应用直接建立 TCP 连接的那个客户端的 IP 地址
        // 不同部署场景下的表现
        // 1、直接访问（无代理/Nginx），获取客户端真实IP
        // 2、通过 Nginx 反向代理，获取 Nginx 的IP，不是客户端真实IP！
        // 3、通过负载均衡器（客户端 → 负载均衡器 → Spring Boot 应用），获取负载均衡器的IP
